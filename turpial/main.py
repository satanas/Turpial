...
class Turpial:
    def __init__(self):
        ...
        parser.add_option('-i', '--interface', dest='interface',
            help='select interface to use %s' % util.available_interfaces(),
            default='cmd')
        ...
        if options.interface in util.INTERFACES.keys():
            self.ui = util.INTERFACES[options.interface](self)
        else:
            print "'%s' is not a valid interface. Availables interfaces are %s" % util.available_interfaces()
            print 'Exit...'
            sys.exit(-1)
