...
class TweetList(gtk.VBox):
    ...
    def __direct_popup_menu(self, widget, event):
        ...
        if (event.button == 3):
            ...
            if rtn.has_key('busy'):
                ...
                copyto.connect('activate', self.__copy_to, "%s: %s" % (user, msg))
    ...
    def __copy_to(self,widget,rm):
        """
        Copying message to memory
        @id: twitt_id
        nhomar at turpialesdepinga dot com
        """
        gtk.clipboard_get().set_text(rm)
    def clear(self):
        self.model.clear()
